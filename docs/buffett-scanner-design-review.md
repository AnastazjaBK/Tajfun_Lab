...
**Status:** v1.29 — **Wszystkie Fazy 0–4 formalnie ukończone i dowiedzione na realnych danych. Faza 5.1 empirycznie potwierdzona i formalnie ukończona.** **Faza 5.2 (historyczny skład S&P 500, OPEN BLOCKER 2): krok 2 (`fja05680/sp500` + ticker→CIK) zakończony — 619/815 rozwiązanych, 100% dzisiejszych 503 aktywnych spółek. Krok 1 (FMP) zakończony — zablokowany planem, właściciel zdecydowała NIE kupować. IVV Proof Run wykonany na realnym koncie — WYNIK NEGATYWNY: mechanizm `asOfDate` opisywany przez narzędzia trzecie nie działa na obecnej stronie ishares.com, zwraca zawsze tę samą stronę HTML zamiast CSV. Jedyny potwierdzony URL (`latest-holdings.csv`, z JSON-LD strony) daje tylko dzisiejszy skład, nie historyczny.** Decyzja o zapisie do `universe_membership` wciąż niepodjęta, decyzja o dalszym kierunku Fazy 5.2 wymaga wyboru właścicielki — patrz „Minimalny Proof Run: IVV (v1.28–v1.29)" niżej. Ten plik jest samodzielny — nie wymaga sięgania do historii commitów.
**Data:** 2026-09-20 (v1.0–v1.4), 2026-09-21 (v1.5–v1.6), 2026-09-24–26 (v1.7–v1.29)
...
**Ocena wpływu na kandydaturę IVV:** to istotnie osłabia IVV jako źródło do walidacji 2012+ — potwierdzony, działający URL daje tylko dzisiejszy skład (wciąż użyteczny jako tani sanity-check „na dziś", podobnie jak Wikipedia, ale nie jako niezależne źródło historyczne). Nie oznacza to, że IVV jest całkowicie wykluczony — mogą istnieć inne, jeszcze nieodkryte mechanizmy dostępu do historii (np. archiwum Wayback Machine starych eksportów CSV, inny endpoint), ale to wymagałoby dalszego researchu, nie zgadywania kolejnego URL-a na chybił trafił.
