...
**Status:** v1.19 — **Wszystkie Fazy 0–4 formalnie ukończone i dowiedzione na realnych danych.** **Faza 5.1 (prototyp point-in-time na SEC XBRL, OPEN BLOCKER 1): pierwsze uruchomienie ujawniło realny błąd tie-breakingu w `value_as_of`, naprawiony i pokryty regresją, oczekuje ponownej empirycznej weryfikacji** — patrz „Status Fazy 5 (v1.19)" niżej. Faza 5.2 (historyczny skład S&P 500, OPEN BLOCKER 2) jeszcze nierozpoczęta — wymaga decyzji o kolejności podejścia (patrz niżej). Ten plik jest samodzielny — nie wymaga sięgania do historii commitów.
**Data:** 2026-09-20 (v1.0–v1.4), 2026-09-21 (v1.5–v1.6), 2026-09-24–25 (v1.7–v1.19)
...
Metoda wybrana: własna warstwa PIT na SEC EDGAR XBRL company-facts (pole `filed`), niezależna od tego, którego komercyjnego dostawcę wybrano do bieżących danych — bez dodatkowego kosztu licencyjnego. **Stan v1.19: kod prototypu (`point_in_time.py`, `cmd_pit_prototype`) uruchomiony na realnym koncie (AAPL/MSFT/KO) — pierwsze uruchomienie ujawniło realny błąd tie-breakingu w `value_as_of` (remis na `filed` wybierał stary okres porównawczy zamiast najnowszego), naprawiony i pokryty testem regresyjnym, oczekuje ponownego uruchomienia „Phase 5.1 Proof Run" na naprawionym kodzie.** Pozostaje do wykonania: (a) potwierdzenie, że po naprawie PIT trafia w sensowne (świeże) okresy dla wszystkich trzech spółek testowych; (b) potwierdzenie jakości/kompletności danych `filed` dla okresu 2009–2012 (obszar niepewny, niezweryfikowany w tym przeglądzie).
...
### Status Fazy 5 (v1.19) — punkt 5.1 (OPEN BLOCKER 1): błąd znaleziony na realnym koncie i naprawiony, oczekuje ponownej weryfikacji
...
**Świadomie pozostająca różnica skali, nie błąd:** nawet po naprawie `pit-prototype` może wciąż pokazywać PIT rzędu 3–4× mniejszy niż FMP — bo XBRL company-facts miesza dane kwartalne (10-Q) i roczne (10-K), więc „najnowszy filed fakt" często trafia na kwartał, podczas gdy `ingest-fundamentals` (Faza 1) domyślnie pobiera `period="annual"`. To widoczne wprost w wypisywanym polu `form=` przy każdej wartości — zamierzona przejrzystość, nie ukryta rozbieżność. Prototyp 5.1 miał udowodnić mechanizm PIT (poprawny wybór faktu wg `filed`), nie dać dokładną rekoncyliację kwartał-do-kwartału — to poza zakresem tego punktu.

**Kod naprawiony, oczekuje ponownego uruchomienia workflow na realnym koncie** — do potwierdzenia: czy po naprawie `end` faktu PIT jest już bliski `filed` (rząd tygodni/miesięcy, nie lat), i czy dla przynajmniej jednej spółki trafiony zostanie świeży kwartał (10-Q) zamiast starego roku porównawczego.

---

## DECYZJE WCIĄŻ WYMAGAJĄCE TWOJEGO WYBORU

**Zero decyzji architektonicznych.** Wszystkie 15 pierwotnych decyzji (D1–D15) są zamknięte, architektura MULTI-USER (v1.5/v1.6) zaakceptowana, **wszystkie Fazy 0–4 w pełni empirycznie zweryfikowane na realnych danych (v1.17)**. Faza 5.1 (kod) zaimplementowana zgodnie z wcześniej ustaloną metodą (SEC EDGAR XBRL); pierwsze uruchomienie na realnym koncie ujawniło błąd tie-breakingu w `value_as_of`, naprawiony w v1.19 — nie wymagało nowej decyzji, tylko wykonania i naprawy na podstawie dowodu z realnego przebiegu.
...
- **Faza 5.2 (OPEN BLOCKER 2 — historyczny skład S&P 500):** do zaproponowania po empirycznym potwierdzeniu 5.1. Wstępna uwaga: darmowy zbiór społecznościowy `fja05680/sp500` (GitHub) nie wymaga płatnego planu FMP ani klucza API i może być szybszą ścieżką walidacji krzyżowej (D3) niż czekanie na potwierdzenie ceny/dostępności `stable/historical-sp-500` u FMP — do przedyskutowania z właścicielem przed rozpoczęciem 5.2, nie decyzja podjęta jednostronnie.
