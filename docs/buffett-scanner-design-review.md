...
**Status:** v1.22 — **Wszystkie Fazy 0–4 formalnie ukończone i dowiedzione na realnych danych. Faza 5.1 (prototyp point-in-time na SEC XBRL, OPEN BLOCKER 1) empirycznie potwierdzona i formalnie ukończona.** **Faza 5.2 (historyczny skład S&P 500, OPEN BLOCKER 2): plan zatwierdzony, krok 2 (analiza `fja05680/sp500`) zaimplementowany i zweryfikowany na realnych danych lokalnie — rozwiązanie ticker→CIK czeka na uruchomienie „Phase 5.2 Proof Run" (blokada sieciowa tej interaktywnej sesji, nie SEC EDGAR)** — patrz „Wykonanie kroku 2 Fazy 5.2 (v1.22)" niżej. Ten plik jest samodzielny — nie wymaga sięgania do historii commitów.
**Data:** 2026-09-20 (v1.0–v1.4), 2026-09-21 (v1.5–v1.6), 2026-09-24–25 (v1.7–v1.22)
...
### Wykonanie kroku 2 Fazy 5.2 (v1.22) — analiza `fja05680/sp500`, DIAGNOSTYCZNE, jeszcze nie zapisane do `universe_membership`
...
**Ticker→CIK: kod gotowy, realne liczby jeszcze NIE uzyskane — blokada środowiskowa, nie logiczna.** `sec.gov` jest zablokowany przez politykę sieciową TEJ interaktywnej sesji (proxy egress) — potwierdzone bezpośrednio (`curl`/`WebFetch` oba zwróciły odmowę na poziomie proxy, nie 404). To ograniczenie tego konkretnego środowiska sandboxowego, nie ograniczenie SEC EDGAR jako takiego — GitHub Actions (runner używany przez wszystkie dotychczasowe „Proof Run") łączył się z `data.sec.gov` bezproblemowo przez całą Fazę 2–5.1. Dlatego realne rozwiązanie 815 tickerów na CIK wymaga uruchomienia „Phase 5.2 Proof Run" na Twoim koncie — kod jest gotowy i przetestowany na syntetycznych/małych realnych przykładach (3 znane spółki: AAPL/MSFT/KO rozwiązane poprawnie lokalnie), ale **skala i lokalizacja `CIK_UNRESOLVED` dla pełnych 815 tickerów jest jeszcze nieznana** — nie zgaduję jej, czekam na realny przebieg.

**Co nadal potrzebujemy od FMP:** bez zmian względem planu v1.21 — jedno zapytanie testowe do `stable/historical-sp-500` (krok 1, wciąż niewykonany, czeka na decyzję o planie Starter) potrzebne do: (a) realnego kształtu JSON, (b) głębokości historycznej, (c) potwierdzenia wymaganego tieru planu. Dopiero po tym możliwe jest właściwe porównanie krzyżowe FMP vs fja05680 z kroku 3 planu.
