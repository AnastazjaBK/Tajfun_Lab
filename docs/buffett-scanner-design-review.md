...
**Status:** v1.10 — **Faza 0 formalnie ukończona i dowiedziona na realnych danych** (workflow „Phase 0 Proof Run", plan Free, 3 spółki: AAPL/MSFT/KO — pełen pipeline init-db→ingest-prices→scan, poprawne CIK-i, poprawne metryki decline scannera). **Faza 1 (fundamenty + silnik wskaźników + logika FLAG/EXCLUDE) zaimplementowana, oczekuje empirycznej weryfikacji na realnym koncie** — patrz „Status Fazy 1 (v1.10)" niżej. Po drodze: naprawiony błąd w kliencie FMP (zła rodzina endpointów `/api/v3` → `/stable`), potwierdzone empirycznie które endpointy wymagają planu Starter (tylko `sp500-constituent`, dokładnie zgodnie z pierwotnym planem — Faza 1/6, nie Faza 0), skorygowana nieścisłość z v1.7. Ten plik jest samodzielny — nie wymaga sięgania do historii commitów.
**Data:** 2026-09-20 (v1.0–v1.4), 2026-09-21 (v1.5–v1.6), 2026-09-24–25 (v1.7–v1.10)
...
### Status Fazy 1 (v1.10) — kod zbudowany, oczekuje empirycznej weryfikacji

Zaimplementowano wszystkie trzy punkty Fazy 1 z sekcji 15:

- **1.1 Ingest fundamentów** — `buffett_scanner/providers/fmp.py` rozszerzony o `get_income_statement`/`get_balance_sheet_statement`/`get_cash_flow_statement` (ścieżki `/stable/income-statement`, `/stable/balance-sheet-statement`, `/stable/cash-flow-statement` — zgadywane na bazie konwencji nazewniczej FMP `/stable/`, **NIEPOTWIERDZONE dokumentacją** z tego samego powodu co w Fazie 0: WebFetch do site.financialmodelingprep.com zablokowany przez proxy sieciowe sesji przez cały czas trwania projektu). `normalize_fundamentals_rows` łączy trzy sprawozdania w format long zgodny z `fundamentals_raw`, mapując znane pola FMP (`revenue`, `netIncome`, `ebitda`, `totalDebt`, `cashAndCashEquivalents`, `totalCurrentAssets`, `totalCurrentLiabilities`, `operatingCashFlow`, `capitalExpenditure`) — brakujące pole jest pomijane, nigdy nie fabrykowane jako 0. Nowa tabela `fundamentals_raw` w `db.py`, dokładnie wg schematu z sekcji 5.
...
**Zero.** Wszystkie 15 pierwotnych decyzji (D1–D15) są zamknięte, architektura MULTI-USER (v1.5/v1.6) zaakceptowana, Faza 0 w pełni empirycznie zweryfikowana (v1.9), Faza 1 zaimplementowana i pokryta testami, oczekuje empirycznej weryfikacji kodu fundamentalnego (v1.10).
