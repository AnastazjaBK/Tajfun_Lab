...
**Status:** v1.30 — **Wszystkie Fazy 0–4 formalnie ukończone i dowiedzione na realnych danych. Faza 5.1 empirycznie potwierdzona i formalnie ukończona.** **Faza 5.2 (historyczny skład S&P 500, OPEN BLOCKER 2 — WCIĄŻ OTWARTY): krok 2 (`fja05680/sp500` + ticker→CIK) zakończony. Krok 1 (FMP) zakończony diagnostycznie — endpoint zablokowany obecnym planem. IVV formalnie ODRZUCONY jako kandydat na niezależną walidację historyczną (potwierdzone negatywnie na realnym koncie, 2026-09-26) — żadnego dalszego reverse-engineeringu.** Właścicielka kontaktuje się z supportem FMP (dostęp do `historical-sp500-constituent`, zakres 2012+, opcja Premium→Starter). **Sesja wstrzymana do czasu odpowiedzi FMP** — patrz „Minimalny Proof Run: IVV (v1.28–v1.29)" niżej. Ten plik jest samodzielny — nie wymaga sięgania do historii commitów.
**Data:** 2026-09-20 (v1.0–v1.4), 2026-09-21 (v1.5–v1.6), 2026-09-24–26 (v1.7–v1.30)
...
**OPEN BLOCKER 2 — Historyczny skład S&P 500 / survivorship bias. WCIĄŻ OTWARTY (stan v1.30).**
Pierwotne założenie (v1.4) o dostawcy FMP przez endpoint `stable/historical-sp-500` było błędne — **prawdziwa nazwa endpointu to `historical-sp500-constituent`** (bez myślnika), potwierdzone empirycznie w Fazie 5.2 krok 1 (v1.26–v1.27): 402 „Restricted Endpoint", ten sam wzorzec co znany z Fazy 0 `sp500-constituent`. Endpoint istnieje, ale jest zablokowany obecnym planem FMP — dokładny wymagany tier nieustalony (komunikat 402 go nie nazywa, strona cennika zablokowana w sesji interaktywnej). Właściciel zdecydowała nie kupować bez tej informacji.

**Krok 2 (fja05680/sp500) wykonany i formalnie zakończony (v1.22–v1.24):** parser + rozwiązanie ticker→CIK przez SEC — 619/815 tickerów rozwiązanych, **100% dzisiejszych 503 aktywnych spółek** rozwiązuje się bezbłędnie. Reszta nierozwiązanych to w większości potwierdzone zdarzenia korporacyjne (przejęcia/fuzje/bankructwa/zmiany nazwy), nie defekt.

**Krok 3 (walidacja krzyżowa) — próba z IVV zakończona niepowodzeniem i ODRZUCONA (v1.28–v1.30):** mechanizm historycznych holdingów iShares (`asOfDate`), opisywany przez dwa niezależne narzędzia trzecie, empirycznie potwierdzony jako NIEDZIAŁAJĄCY na obecnej wersji ishares.com (zwraca zawsze tę samą stronę HTML, nie CSV) — właściciel zdecydowała nie kontynuować dalszego reverse-engineeringu.

**Stan na dziś: brak drugiego, niezależnego źródła do walidacji krzyżowej fja05680/sp500 dla 2012+.** Właścicielka kontaktuje się bezpośrednio z supportem FMP w sprawie dostępu do `historical-sp500-constituent`, potwierdzenia zakresu 2012+, oraz możliwości wykupienia Premium na jeden okres rozliczeniowy z późniejszym downgrade'em do Starter. **Blocker pozostaje formalnie otwarty do czasu tej odpowiedzi — dalsza praca nad Fazą 5.2 (w tym decyzja o zapisie do `universe_membership`) wstrzymana.**
...
- **Faza 5.2 (OPEN BLOCKER 2 — historyczny skład S&P 500), stan v1.30: WSTRZYMANA.** Krok 2 (fja05680/sp500) ukończony (619/815 CIK, 100% dzisiejszych spółek). Krok 1 (FMP) i krok 3 (IVV) oba wykonane empirycznie — FMP zablokowany planem bez znanego tieru, IVV formalnie odrzucony (mechanizm historyczny nie działa, potwierdzone na realnym koncie). **Właścicielka kontaktuje się z supportem FMP** w sprawie dostępu do `historical-sp500-constituent`, zakresu 2012+, i opcji Premium→Starter. Sesja wstrzymana do czasu tej odpowiedzi — żadna dalsza praca nad Fazą 5.2 (w tym decyzja o `universe_membership`) do tego czasu.
