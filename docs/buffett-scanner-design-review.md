...
**Status:** v1.11 — **Faza 0 I Faza 1 formalnie ukończone i dowiedzione na realnych danych** (workflow „Phase 1 Proof Run", plan Free, 3 spółki: AAPL/MSFT/KO — pełen pipeline init-db→ingest-prices→ingest-fundamentals→prefilter, poprawne CIK-i, poprawne wskaźniki fundamentalne, poprawnie zadziałana logika FLAG bez blokowania — patrz „Status Fazy 1 (v1.11)" niżej). Po drodze: naprawiony błąd w kliencie FMP (zła rodzina endpointów `/api/v3` → `/stable`), potwierdzone empirycznie które endpointy wymagają planu Starter (tylko `sp500-constituent`), potwierdzony i naprawiony limit `limit<=5` na planie Free dla endpointów fundamentalnych, skorygowana nieścisłość z v1.7. Ten plik jest samodzielny — nie wymaga sięgania do historii commitów.
**Data:** 2026-09-20 (v1.0–v1.4), 2026-09-21 (v1.5–v1.6), 2026-09-24–25 (v1.7–v1.11)
...
**Zero.** Wszystkie 15 pierwotnych decyzji (D1–D15) są zamknięte, architektura MULTI-USER (v1.5/v1.6) zaakceptowana, Faza 0 i Faza 1 w pełni empirycznie zweryfikowane na realnych danych (v1.11).
